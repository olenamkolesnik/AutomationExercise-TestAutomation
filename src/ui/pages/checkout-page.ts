...
export class CheckoutPage extends BasePage {
  ...
  private readonly orderSummaryRoot = this.cartTable.getByRole('row', {
    name: 'Total Amount',
  });
  ...
  async expectPageOpened() {
    ...
    logger.info(
      'Checkout page is opened with correct URL and cart table is visible',
    );
  }

  async getCartItemByName(name: string) {
    logger.info(
      `Looking for cart item with name "${name}" in the checkout page`,
    );
